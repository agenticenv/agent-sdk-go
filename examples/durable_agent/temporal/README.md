...
> **Task batch note:** `task examples:*` sets `EXAMPLES_AUTO_APPROVE=true`, which skips `DisableLocalWorker` so this lab can run single-process in CI. Manual runs keep the split.

The agent also demonstrates reconnect end to end — it persists `runID` and the last event offset to `/tmp/durable_agent_temporal_runstate.json` on every event. Kill the process mid-stream (`pkill -SIGKILL`) and restart **while the worker is still processing**: the agent detects the saved state, asks whether to reconnect, and resumes the stream from the exact offset where it stopped via `GetAgentStream` + `WithOffset`. See **Scenario 5c** below for the full walkthrough.
...
saveRunState(runID, 0, prompt)          // persisted to /tmp/durable_agent_temporal_runstate.json
...
See [`agent_with_reconnect/`](../../agent_with_reconnect/) for a focused end-to-end reconnect demo outside this REPL.

## Prerequisites

This example **always uses Temporal** — shared opts wire `temporal.WithTemporalConfig` directly. For the Restate durability lab, see [`../restate/`](../restate/).
...
> **Temporal Web UI** — while the worker and agent are running, open [http://localhost:8233](http://localhost:8233) to inspect workflows and activities (namespace `default`, task queue `agent-sdk-go-durable-agent_remote-worker` by default). Useful when stepping through the durability scenarios below.
...
```bash
# Terminal 1 — worker
go run ./durable_agent/temporal/worker

# Terminal 2 — agent (one-shot)
go run ./durable_agent/temporal/agent "Hello from remote agent!"
```
...
>
> ```bash
> pkill -SIGKILL -f 'go run ./durable_agent/temporal/worker|go-build/.*/worker' 2>/dev/null; pkill -SIGKILL -f 'go run ./durable_agent/temporal/agent|go-build/.*/agent' 2>/dev/null; true
> ```
>
...
```text
Agent worker starting on task queue "agent-sdk-go-durable-agent_remote-worker". Run this before the agent.
...
```bash
go run ./durable_agent/temporal/agent
```
...
```text
[error] failed to start stream: no workers available on task queue agent-sdk-go-durable-agent_remote-worker
...
```bash
pkill -SIGKILL -f 'go run ./durable_agent/temporal/worker|go-build/.*/worker'
```
...
```bash
pkill -SIGKILL -f 'go run ./durable_agent/temporal/agent|go-build/.*/agent'
```
...
```bash
cat /tmp/durable_agent_temporal_runstate.json
```
...
**Stop any worker on the default queue first** (cleanup command above). Point the **worker** at a different queue via env (no code edits). Agent keeps the default `agent-sdk-go-durable-agent_remote-worker` from `.env`:

**Terminal 1:**

```bash
TEMPORAL_TASKQUEUE=wrong-queue-durable-agent go run ./durable_agent/temporal/worker
```
...
> **Note:** Same timing as scenario 2 — the agent checks pollers on `agent-sdk-go-durable-agent_remote-worker`, not the worker’s queue. With no pollers there (~**15 seconds**), path **A** is typical. Stale pollers on the agent queue from an earlier run can produce path **B** (**3 minutes** run timeout).
...
Check the Temporal Web UI — a workflow may appear on `agent-sdk-go-durable-agent_remote-worker` even though the only running worker polls a different queue.
