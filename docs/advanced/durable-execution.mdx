...
description: "How agent runs survive crashes — automatic runtime resilience and client-side Run / Stream recovery"
---

<Note>
**Durable by default, on every runtime.** The in-process (local) runtime is durable via [durable-go](https://github.com/agenticenv/durable-go) — no config needed, no external server. [Temporal](/runtimes/temporal) and [Restate](/runtimes/restate) add horizontal scaling and a client/worker split on top of the same durability guarantees, plus token-level (not step-level) reconnect fidelity. See [Runtime support](#runtime-support) for the one fidelity difference on local.
</Note>
...
| **Client-side stream recovery** | Your subscriber process can reconnect after a crash and resume the event stream from exactly where it left off | Your code — `GetAgentStream` (+ `WithOffset` on Temporal/Restate) |
...
This is automatic on every runtime — durable by default on local, no SDK changes needed. Temporal/Restate are additional opt-in choices, not what makes a run durable.

| Runtime | How steps are recorded | Who executes after a crash |
|---|---|---|
| **Local** | durable-go journal on local disk (`DataDir`) | The next process that reconnects to the same run — there is no separate server, so nothing resumes an abandoned run until you do (see the [local durability lab](/examples/durable-agent-local)) |
...
The local journal is **plaintext JSON** by default (user prompt, LLM content, tool args and results). Do not put secrets or raw PII in prompts, tool I/O, or errors. For at-rest encryption, a journal MAC, or approval tokens that survive a restart, pass a caller-owned [`durable.Engine`](/runtimes/in-process#caller-owned-engine-payload-codec-journal-mac-step-token-key) on `local.LocalConfig.Engine` — same pattern as Temporal Cloud / TLS via `WithTemporalClient`. See [durable-go data privacy](https://github.com/agenticenv/durable-go#data-privacy--sensitive-payloads).
...
On local, this shows up differently on reconnect: instead of clean live token delivery for an already-completed step, you get one coalesced `step_replayed` custom event per completed step (see [Client-side stream recovery](#client-side-stream-recovery) below).

### Experiencing it

The [Durable Agent (Local)](/examples/durable-agent-local) example is the fastest way to feel this — zero infrastructure, kill and restart the same process, reconnect via `GetAgentStream` (no `WithOffset` — see [Runtime support](#runtime-support)).
...
`ErrRunNotFound` / `ErrStreamNotFound` — returned when `runID` is unknown, or on the local runtime when durability is off (`local.DurabilityOff()`) so there is no journal to reconnect to.

`ErrStreamOffsetNotSupported` — returned by the local runtime for any `fromOffset > 0` (no per-token durable log — only step-level replay from the start).

## Runtime support

| Runtime | Server-side durability | Client-side reconnect |
|---|---|---|
| Local | Automatic (default) — durable-go journal on local disk | `GetAgentStream` / `GetAgentRun` — no `WithOffset(n>0)`; reconnect replays step history from the start, then goes live |
