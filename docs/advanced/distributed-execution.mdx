...
| Hook group names | Fingerprint includes lifecycle hook group names |
| Named LLM clients | Extra clients: names plus each client's model and provider |
| Error control | Hook slots (not bodies), `FallbackLLMClient` name, breaker thresholds |
