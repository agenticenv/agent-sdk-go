...
<Note>
**Temporal runtime only.** Running a separate worker process requires the [Temporal runtime](/runtimes/temporal). The `LocalRuntime` executes everything in-process and does not support `NewAgentWorker`.
</Note>
