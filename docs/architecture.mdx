...
By default `NewAgent` embeds a local worker in the same process — no separate binary needed for development. For production, split client and worker — see [Distributed execution](/advanced/distributed-execution).
