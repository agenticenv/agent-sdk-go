...
```bash
# agctl is its own module in cmd/ (build with Task)
cd cmd
task build
./bin/agctl
...
|---|---|---|
| **Install** | `go get github.com/agenticenv/agent-sdk-go@latest` | Download release binary or `cd cmd && task build` |
...
- Default: `cmd/logs/agctl.log` (gitignored)
