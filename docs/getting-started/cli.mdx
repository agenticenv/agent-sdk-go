---
title: "CLI"
description: "Run agents interactively from the terminal using agctl with a YAML config file"
---

**`agctl`** is the interactive chat session (conversation mode) for Agent SDK for Go. Type prompts and receive agent responses. It uses the same SDK as your application code and supports both in-process and Temporal runtimes via a YAML config file.

<Note>
**`agctl` is not the Go library.** Install the SDK with `go get github.com/agenticenv/agent-sdk-go`. Install `agctl` separately — download a release binary or build from source.
</Note>

## Install agctl

### Option 1 — Release binary (recommended)

Pre-built binaries for Linux, macOS, and Windows (amd64 and arm64) are on [GitHub Releases](https://github.com/agenticenv/agent-sdk-go/releases).

1. Open the latest release
2. Download the archive for your platform from **Assets**
3. Extract and add `agctl` to your `PATH`

### Option 2 — Build from source

```bash
# agctl is its own module in cli/ (build with Task)
cd cli
task build
./bin/agctl

# Or install to $GOPATH/bin:
task install
agctl -config config.yaml
```

### Option 3 — Run without building (development)

From a cloned repository:

```bash
cd cli
go run .
```

Compiles and runs on each invocation — useful during development.

## Quick start

```bash
# 1. Copy the sample config
cp cli/config.sample.yaml config.yaml
# Edit config.yaml — set llm.apiKey or use AGENT_LLM_APIKEY

# 2. Run the CLI
agctl -config config.yaml
```

If you use a release binary without cloning the repo, copy `config.sample.yaml` from the release assets into your working directory.

Type prompts at the `>` prompt. Enter `exit`, `quit`, or `bye` to end the session.

## Configuration file

| File | Purpose |
|---|---|
| `cli/config.sample.yaml` | Template — safe to commit |
| `cli/config.yaml` | Your config — gitignored, copy from sample |

Run with a custom path:

```bash
agctl -config /path/to/config.yaml
```

### Runtime selection

Set `runtime` in `config.yaml`:

| Value | Behavior |
|---|---|
| `local` (default) | In-process — no Temporal server required |
| `temporal` | Durable execution — requires a running Temporal server |

Override per invocation:

```bash
AGENT_RUNTIME=temporal agctl -config config.yaml
```

### Minimal config.yaml

```yaml
runtime: local

llm:
  provider: openai
  apiKey: ""           # prefer AGENT_LLM_APIKEY env var
  model: gpt-4o
  baseURL: https://api.openai.com/v1

temporal:
  host: localhost
  port: 7233
  namespace: default
  taskQueue: agent-sdk-go

logger:
  level: info
  output: logs/agctl.log
  format: json
```

## Environment variables

Environment variables override config file values. Prefer them for secrets.

| Variable | Description |
|---|---|
| `AGENT_RUNTIME` | `local` or `temporal` |
| `AGENT_TEMPORAL_HOST` | Temporal server host |
| `AGENT_TEMPORAL_PORT` | Temporal gRPC port |
| `AGENT_TEMPORAL_NAMESPACE` | Temporal namespace |
| `AGENT_TEMPORAL_TASKQUEUE` | Task queue name |
| `AGENT_LLM_PROVIDER` | `openai`, `anthropic`, or `gemini` |
| `AGENT_LLM_APIKEY` | LLM API key |
| `AGENT_LLM_MODEL` | Model name |
| `AGENT_LLM_BASEURL` | Optional base URL for OpenAI-compatible proxies |
| `AGENT_LOGGER_LEVEL` | `error`, `warn`, `info`, `debug` |
| `AGENT_LOGGER_OUTPUT` | Log file path, `stdout`, or `stderr` |
| `AGENT_SHOW_LLM_USAGE` | Print session token usage summary on exit |

```bash
export AGENT_LLM_APIKEY=sk-your-key
export AGENT_LLM_PROVIDER=openai
export AGENT_LLM_MODEL=gpt-4o
agctl -config config.yaml
```

## MCP servers (optional)

Add `mcp.servers` entries to `config.yaml` to connect MCP tool servers. When at least one server is enabled, the CLI registers [`WithMCPConfig`](/features/mcp) and [`AutoToolApprovalPolicy`](/features/approvals) so MCP tools run without per-call approval in the interactive session.

```yaml
mcp:
  servers:
    - enabled: true
      name: filesystem
      transport: stdio
      command: npx
      args: ["-y", "@modelcontextprotocol/server-filesystem", "/tmp"]
```

Each entry supports: `enabled`, `name`, `transport` (`stdio` or `streamable_http`), `command`/`args` (stdio) or `url`/`bearer_token` (HTTP), and optional `timeout_seconds`, `retry_attempts`, `allow_tools`, `block_tools`.

See `cli/config.sample.yaml` for HTTP transport, OAuth, and tool filter examples.

## Token usage summary

When `show_llm_usage: true` is set in `config.yaml` (or `AGENT_SHOW_LLM_USAGE=true`), the CLI accumulates `LLMUsage` across every turn of the session and prints one summary on exit (`exit`/`quit`/`bye`):

```
[USAGE] session total
  prompt_tokens:     12120
  completion_tokens: 262
  total_tokens:      12382
  cached_prompt:     12541
```

Use this to measure cost across a full interactive session. `cached_prompt` reflects provider cache hits when the LLM reports them (for Anthropic, only when prompt caching is enabled — the CLI enables it automatically for Anthropic).

## SDK vs CLI

| | SDK (library) | CLI (`agctl`) |
|---|---|---|
| **Install** | `go get github.com/agenticenv/agent-sdk-go@latest` | Download release binary or `cd cli && task build` |
| **Use** | Import `pkg/agent` in your Go application | Interactive chat session from the terminal |
| **Config** | Go code — `NewAgent(...)` options | YAML file + env vars |

Both are published from the same repository. Pin the SDK version in your `go.mod`; pin the CLI by downloading the matching release tag.

## Logging

`agctl` prints **only user prompts and agent responses** on the console. Internal SDK logs go to a file.

- Default: `cli/logs/agctl.log` (gitignored)
- Override: `logger.output` in config or `AGENT_LOGGER_OUTPUT`

## Related

<CardGroup cols={2}>

  <Card title="Quickstart" icon="bolt" href="/getting-started/quickstart" horizontal>
    Build an agent in your own Go application
  </Card>

  <Card title="Configuration" icon="sliders" href="/getting-started/configuration" horizontal>
    All SDK options the CLI uses under the hood
  </Card>
</CardGroup>
