...
| `a.Close` | Stops any embedded Temporal worker or Restate SDK endpoint and flushes OTLP exporters; active durable runs are **not** terminated |

<Warning>
`defer a.Close()` is not optional when using a durable runtime — it shuts down the embedded Temporal worker or Restate SDK endpoint. On the in-process runtime it is a no-op but still good practice.

**Durability note:** `Close` does not terminate in-flight Temporal workflows or Restate invocations. Runs continue on the server and resume when a worker or endpoint is available again — this is the core durability guarantee.
</Warning>
