...
Hooks let you register middleware callbacks that fire at key points in the agent execution lifecycle. Register once with [`WithHooks`](/getting-started/configuration) — the SDK calls your functions automatically across `Run` and `Stream`.

Lifecycle hooks inspect, mutate, or abort an operation. They are **not** error-control. For model fallback after LLM retries, extra iterations, or skipping a looping tool, use [`WithErrorControl`](/features/error-control).
...
**Hooks:** `BeforeLLM` (cache read / short-circuit), `AfterLLM` (per-call tracking, cache write). Model fallback after a failed Generate is [`OnLLMFailure`](/features/error-control), not `AfterLLM`.
...
| Model fallback on LLM error | [`OnLLMFailure`](/features/error-control) (not `AfterLLM`) |
...
<CardGroup cols={2}>
  ...
  <Card title="Error Control" icon="play" href="/examples/error-control" horizontal>
    Fallback, extend iterations, breaker — separate from WithHooks
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Error Control" icon="shield-halved" href="/features/error-control" horizontal>
    Fallback, extend iterations, and circuit breaker — after retries
  </Card>
</CardGroup>
