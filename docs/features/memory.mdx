...
agentRun, _ := a.Run(ctx, "Remember I prefer concise answers.", nil)
...
Pass the **same** `memory.Config` to both `NewAgent` and `NewAgentWorker`. Both processes must reach the same backend host. See [Distributed Execution](/advanced/distributed-execution) for the full split-process setup.
