...
agentRun, err := a.Run(ctx, "I'm Alice. Remember that.", opts)
result, err := agentRun.Get(ctx)

agentRun, err = a.Run(ctx, "What's my name?", opts)
result, err = agentRun.Get(ctx) // receives: "Alice"
```
...
Use Redis on both the agent and worker processes. Pass the same `conversation.Config` to `NewAgent` and `NewAgentWorker`. Set `SaveOnIteration: true` on the worker if external consumers need live history mid-run. See [Distributed Execution](/advanced/distributed-execution) for the full setup.
