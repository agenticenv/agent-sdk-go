...
|---|---|---|
| Agent run timeout | Entire `Run` / `Stream` call | `WithTimeout` or context deadline |
...
## Relationship to error control

[`WithErrorControl`](/features/error-control) hooks run **after** these retries. Do not stack a hook-level retry on top of `MaxAttempts`. `OnLLMFailure` sees the error from the last attempt; `OnMaxIterationsExceeded` is independent of per-operation retries.
...
<CardGroup cols={2}>
  ...
  <Card title="Error Control" icon="shield-halved" href="/features/error-control" horizontal>
    Fallback and extend-iterations after retries exhaust
  </Card>
