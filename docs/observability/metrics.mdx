...
Emitted by `Agent.Run` and `Agent.Stream`:

| Metric | Kind | Description |
|---|---|---|
| `agent.run.started` | counter | Each `Run` invocation |
...
| `agent.tool.call.unknown` | counter | LLM named a tool that is not registered; run continues. In-process and Restate only — Temporal logs, no counter |
| `agent.tool.call.circuit_open` | counter | Circuit breaker skipped the tool; run continues. In-process and Restate only — Temporal logs, no counter |
