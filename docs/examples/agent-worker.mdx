---
title: "Agent Worker"
description: "Split the agent client and Temporal worker into separate processes using DisableLocalWorker and NewAgentWorker"
---

<Note>
**Temporal runtime.** `NewAgentWorker` and `DisableLocalWorker` apply to the [Temporal runtime](/runtimes/temporal). For Restate durability labs, see [Durable Agent (Restate)](/examples/durable-agent-restate).
</Note>

Minimal **agent + worker** split: worker polls the task queue; agent process starts runs with `DisableLocalWorker`.

Source: [`examples/agent_with_worker/`](https://github.com/agenticenv/agent-sdk-go/tree/main/examples/agent_with_worker)

## What it demonstrates

- `NewAgentWorker` with the same options as `NewAgent`
- `DisableLocalWorker` on the client process
- `Stream` from the remote agent process
- Foundation pattern for production distributed agents

## Run

From `examples/` with Temporal running:

```bash
task infra:temporal:up && task infra:temporal:wait

# Terminal 1
AGENT_RUNTIME=temporal go run ./agent_with_worker/worker

# Terminal 2
AGENT_RUNTIME=temporal go run ./agent_with_worker/agent "Hello from remote agent!"
```

For interactive durability scenarios (kill/restart), see [Durable Agent (Temporal)](/examples/durable-agent).

## Key code

**worker/main.go** — polls Temporal, executes workflows and activities:

```go
w, err := agent.NewAgentWorker(
    agent.WithName("agent-worker"),
    agent.WithLLMClient(llmClient),
    temporal.WithTemporalConfig(temporalCfg),
)
w.Start()
```

**agent/main.go** — starts runs, consumes stream without a local worker:

```go
a, err := agent.NewAgent(
    agent.WithName("agent-worker"),
    agent.WithLLMClient(llmClient),
    temporal.WithTemporalConfig(temporalCfg),
    agent.DisableLocalWorker(),
)
// runID is available synchronously — persist for GetAgentStream before consuming events.
agentStream, _ := a.Stream(ctx, prompt, nil)
runID := agentStream.ID()
eventCh, _ := agentStream.Events(ctx)
_ = runID
_ = eventCh
```

Both processes use identical agent options so the SDK fingerprint matches.

## Expected output

Terminal 1 (worker) shows Temporal polling logs at `LOG_LEVEL=info`.

Terminal 2 (agent) prints the streamed reply:

```
Hello from remote agent! I'm running as a Temporal workflow on a separate worker process.
```

See [Distributed Execution](/advanced/distributed-execution) and [Temporal Runtime](/runtimes/temporal) for production context.

## Learn more

<CardGroup cols={2}>

  <Card title="Distributed Execution" icon="server" href="/advanced/distributed-execution" horizontal>
    Production split-process guide
  </Card>

  <Card title="Durable Agent (Temporal)" icon="shield" href="/examples/durable-agent" horizontal>
    Advanced durability scenarios
  </Card>
</CardGroup>