...
Use it after [Restate runtime](/runtimes/restate) when you need to **feel** Restate durability. For a zero-infrastructure single-process lab, see [Durable Agent (Local)](/examples/durable-agent-local); for the Temporal split worker/agent lab, see [Durable Agent (Temporal)](/examples/durable-agent).
