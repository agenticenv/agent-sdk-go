...
Uses `temporal.WithTemporalClient` instead of `temporal.WithTemporalConfig` (from `pkg/agent/runtime/temporal`). You create and own the Temporal SDK client — required for TLS, API key auth, Temporal Cloud, and custom connection options.
...
a, err := agent.NewAgent(
    temporal.WithTemporalClient(tc, "my-task-queue"),
