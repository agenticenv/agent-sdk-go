...
Verify schema: `curl -s http://localhost:8081/v1/schema | jq '.classes[].class'` — expect `Document` and `AgentMemory` (examples compose publishes Weaviate on host **8081**; Restate uses **8080**).
