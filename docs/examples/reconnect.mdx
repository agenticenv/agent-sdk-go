...
The simulated crash cancels only the Events context (not Stream), not a real `kill -9`. For a real process-kill story, see [Durable Agent (Local)](/examples/durable-agent-local) (zero infrastructure), [Durable Agent (Temporal)](/examples/durable-agent), or [Durable Agent (Restate)](/examples/durable-agent-restate).
...
<CardGroup cols={2}>
  ...
  <Card title="Durable Agent (Local)" icon="laptop" href="/examples/durable-agent-local" horizontal>
    Real process kill, zero infrastructure, no offsets
  </Card>
