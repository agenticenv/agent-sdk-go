...
Agent that calls `Stream` and emits lifecycle events as the loop runs — text deltas, tool calls, run finished.
...
// runID is available synchronously — save it for reconnect before consuming events.
agentStream, err := a.Stream(ctx, prompt, nil)
if err != nil {
    log.Fatal(err)
}
runID := agentStream.ID()
_ = runID // persist alongside your session key when crash-durability matters
